...
#include "usbcdc_interface.h"   // DS added
...
/* Public variables -----------------------------------------------------------*/
volatile uint8_t USBCDC_Cmd;
volatile uint8_t USBCDC_Cmd_Pending = 0;
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
extern __IO uint64_t SysTickCounts;    // DS added
extern uint16_t SysTickFrequency;
...
* the message is sent if use_timeout=0. 
*/
...
uint16_t USBCDC_Send_Data(uint8_t* data, uint32_t size, uint8_t use_timeout)
{
  ...
  {
    SysTickMem = SysTickCounts;
    while(Rx_length > (APP_RX_DATA_SIZE-2))
    {    
      Rx_length = (int32_t)APP_Rx_ptr_in - (int32_t)APP_Rx_ptr_out;
      if (Rx_length < 0) Rx_length = APP_RX_DATA_SIZE+Rx_length;
      if(use_timeout){
        if ((SysTickCounts-SysTickMem)/SysTickFrequency >= 1){
          return(0);                                          // 1 sec timout
        }
      }
    }
...
void USBCDC_Config(void)
{
  ...
  NVIC->IP[67] = 0x70;    // Put USB (OTG_FS) interrupt in the middle   
}
...
static uint16_t VCP_DataRx (uint8_t* Buf, uint32_t Len)
{
  ...
  for (i = 0; i < Len; i++)
  {
    if (USBCDC_Cmd_Pending==0)
    {
      switch(index) 
      {
        case -1:                                // looking for first byte
          if (*(Buf + i)==msg_frame[0]) index++;
          else index = -1;
          break;
        case 0:                                 // looking for 2nd byte
          if (*(Buf + i)==msg_frame[1]) index++;
          else index = -1;
          break;
        case 1:                                 // looking for 3rd byte
          if (*(Buf + i)==msg_frame[2]) index++;
          else index = -1;
          break;
        case 3:                                 // looking for end byte
          if (*(Buf + i)==msg_frame[4])
          {
            index=-1;
            USBCDC_Cmd = cmd;
            USBCDC_Cmd_Pending = 1;
          }
          else index = -1;
          break;
        default:                                 // part of msg body
          cmd = *(Buf+i);  
          index++;
      }
    }
